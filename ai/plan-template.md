Use the following instructions to generate a detailed, step-by-step `plan.md`
file with Markdown checkboxes for building the app we've been discussing.

You are an AI tasked with creating a detailed implementation plan for building
the app [APP NAME] using Wasp. Generate a `plan.md` file that outlines the
development phases and specific tasks. **Crucially, format every individual task
and sub-task as a standard Markdown checklist item (e.g.,
`- [ ] Task Description`)** to allow for easy progress tracking.

The plan should include the following sections, each broken down into specific,
...
Replace placeholders (e.g., `[APP NAME]`, `<Entity 1>`) with project-specific
details. Ensure every task is a distinct checklist item (`- [ ]`). The goal is a
