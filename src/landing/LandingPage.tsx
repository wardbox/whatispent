...
export default function Landing() {
  ...
  } = useQuery(getInstitutions, undefined, {
    enabled: !!user,
  })
  ...
  const {
    ...
    error: summaryError,
  } = useQuery(getSpendingSummary, undefined, {
  ...
  const {
    ...
    error: categoryError,
  } = useQuery(getCategorySpending, undefined, {
    enabled: !!user && !isLoadingInstitutions,
  })
  ...
  const handleSync = async (institutionId: string, institutionName: string) => {
    ...
    try {
      ...
      toast.success(
        `Synced ${result.syncedTransactions} transactions for ${institutionName}.`,
      )
      refetchInstitutions()
    } catch (error: any) {
      console.error('Sync failed:', error)
      const errorMessage =
        error?.message || 'Failed to sync transactions. Please try again later.'
      ...
      <motion.section variants={fadeIn} className='space-y-4'>
        ...
        {!user ? (
          <p className='mt-8 text-lg text-muted-foreground'>
            Please log in or sign up to manage your accounts.
          </p>
        ) : isLoadingInstitutions ? (
          <p>Loading connected accounts...</p>
        ) : institutionsError ? (
          <p className='text-red-500'>
            Error loading accounts: {institutionsError.message}
          </p>
          ...
          <div className='mt-8 border-t pt-8'>
            ...
            <ul className='space-y-4'>
              {institutions.map(inst => {
                ...
                return (
                  <li key={inst.id} className='space-y-3 rounded-md border p-4'>
                    <div className='flex flex-wrap items-center justify-between gap-2'>
                      ...
                      <div className='flex items-center gap-2'>
                        ...
                        <Button
                          ...
                          variant='outline'
                          onClick={() =>
                            handleSync(inst.id, inst.institutionName)
                          }
                          disabled={isSyncing}
                        >
          ...
          <div className='mt-8 border-t pt-8'>
            ...
            <p className='mb-4 text-muted-foreground'>
              Link your financial institutions securely via Plaid.
            </p>
            <PlaidLinkButton
              createLinkTokenAction={createLinkToken}
              exchangePublicTokenAction={exchangePublicToken}
          ...
          >
                ...
                {spendingSummary && (
                  <ul className='space-y-2'>
                    <li>
                      Today:{' '}
                      <span className='font-medium'>
                        {formatCurrency(spendingSummary.today.amount)}
                      </span>
                    </li>
                    <li>
                      This Week:{' '}
                      <span className='font-medium'>
                        {formatCurrency(spendingSummary.thisWeek.amount)}
                      </span>
                    </li>
                    <li>
                      This Month:{' '}
                      <span className='font-medium'>
                        {formatCurrency(spendingSummary.thisMonth.amount)}
                      </span>
                    </li>
                  </ul>
                )}
            ...
            <Card>
              ...
              <CardContent>
                ...
                {monthlyError && (
                  <p className='text-red-500'>Error: {monthlyError.message}</p>
                )}
            ...
            <Card>
              ...
              <CardContent>
                ...
                {categoryError && (
                  <p className='text-red-500'>Error: {categoryError.message}</p>
                )}
