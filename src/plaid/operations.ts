...
import { type Transaction, type Institution, type Account } from '@prisma/client';
...
export const exchangePublicToken = (async (
  ...
  try {
    ...
    throw new HttpError(500, 'Failed to exchange public token and save institution.');
  }
}) satisfies ExchangePublicToken<ExchangePublicTokenPayload, ExchangePublicTokenResult>;
...
export const syncTransactions = (async (
  ...
  if (!institution) {
    throw new HttpError(404, 'Institution not found or does not belong to user.');
  }
  if (!institution.accounts || institution.accounts.length === 0) {
    console.warn(`No accounts found for institution ${institutionId}. Skipping sync.`);
  ...
  if (dayjs(startDate).isBefore(twoYearsAgo)) {
    console.warn(`Start date ${startDate} is more than 2 years ago. Adjusting to ${twoYearsAgo}.`);
    startDate = twoYearsAgo;
  }

  const endDate = dayjs().format('YYYY-MM-DD'); // Today

  console.log(`Syncing transactions for institution ${institutionId} from ${startDate} to ${endDate}`);
  ...
  try {
    ...
    if (plaidTransactions.length === 0) {
      console.log(`No new transactions found for institution ${institutionId} in the date range.`);
    ...
    const transactionData = plaidTransactions.map((tx) => {
      const internalAccountId = accountIdMap.get(tx.account_id);
      if (!internalAccountId) {
        console.warn(`Skipping transaction ${tx.transaction_id}: Cannot find internal account for Plaid account ${tx.account_id}`);
      ...
      const createData = {
        ...
        category: tx.personal_finance_category?.primary ? [tx.personal_finance_category.primary] : [], // Ensure it's always string[]
    ...
    console.log(`Successfully synced ${transactionData.length} transactions for institution ${institutionId}. Last sync updated to ${updatedInstitution.lastSync}`);
    ...
    console.error(`Error syncing transactions for institution ${institutionId}:`, error.message);
    if (error.response?.data?.error_code === 'ITEM_LOGIN_REQUIRED') {
       // Log or handle specific Plaid errors like login required
       console.warn(`Plaid item login required for institution ${institutionId}. Sync failed.`);
       // Optionally update institution status here
       throw new HttpError(400, `Connection requires update for institution ${institution.institutionName}. Please reconnect.`);
    }
    // Rethrow a generic error
    throw new HttpError(500, `Failed to sync transactions for institution ${institution.institutionName}.`);
  }
...
export const getSpendingSummary: GetSpendingSummary<void, SpendingSummary> = async (_args, context) => {
  if (!context.user) {
    throw new HttpError(401);
  }
...
export const getMonthlySpending: GetMonthlySpending<MonthlySpendingArgs, MonthlySpendingEntry[]> = async (args, context) => {
  if (!context.user) {
    throw new HttpError(401);
  }
  ...
  const startDate = dayjs.utc().subtract(numMonths - 1, 'month').startOf('month').toDate();
  ...
  for (let i = 0; i < numMonths; i++) {
    const monthKey = dayjs.utc().subtract(numMonths - 1 - i, 'month').format('YYYY-MM');
    result.push({
      month: monthKey,
...
export const getCategorySpending: GetCategorySpending<void, CategorySpendingEntry[]> = async (_args, context) => {
  if (!context.user) {
    throw new HttpError(401);
  }
  ...
  transactions.forEach(tx => {
    ...
    if (primaryCategory) {
      const amount = Math.abs(tx.amount);
      categoryTotals[primaryCategory] = (categoryTotals[primaryCategory] || 0) + amount;
    }
  });
...
export const getAllTransactions: GetAllTransactions<AllTransactionsArgs, TransactionWithDetails[]> = async (args, context) => {
    if (!context.user) {
        throw new HttpError(401);
    }
...
type GetInstitutionsResult = (Pick<Institution, 'id' | 'institutionName' | 'lastSync' | 'plaidInstitutionId'> & {
 ...
 */
export const getInstitutions: GetInstitutions<void, GetInstitutionsResult> = async (
  _args,
  context
) => {
  if (!context.user) {
