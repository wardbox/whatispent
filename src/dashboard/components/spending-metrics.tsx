...
export function SpendingMetrics() {
    ...
    return (
      <AnimatePresence>
        <motion.div className="grid gap-8 md:grid-cols-3">
          {[...Array(3)].map((_, index) => (
            <motion.div key={index} className="h-20 animate-pulse rounded-lg bg-muted"></motion.div>
          ))}
        </motion.div>
      </AnimatePresence>
    );
  ...
  if (summaryError) {
    return <div className="text-red-500">Error loading spending summary: {summaryError.message}</div>;
  }

  if (!spendingSummary) {
    return <div className="text-center text-gray-500">No spending data available yet.</div>;
  }
  ...
  return (
    <div className="grid gap-8 md:grid-cols-3">
      {metrics.map((metric, index) => (
        ...
        >
          <div className="text-xs font-light text-muted-foreground">{metric.title}</div>
