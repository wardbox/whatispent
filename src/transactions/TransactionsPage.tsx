...
import { ArrowLeft, MagnifyingGlass, SlidersHorizontal, SortAscending } from "@phosphor-icons/react"
...
export default function TransactionsPage() {
  const { data: transactions, isLoading: isLoadingTransactions, error: transactionsError } = useQuery(getAllTransactions)
    ...
    <div className="flex min-h-screen w-full flex-col bg-background">
      <header className="sticky top-0 z-10 flex h-16 items-center justify-between border-b border-border bg-background/90 px-6 backdrop-blur-md">
        <div className="flex items-center gap-2">
          ...
          <span className="text-lg font-light tracking-tight text-foreground">Transactions</span>
        </div>
      </header>
            ...
            <div className="flex items-center gap-2">
              ...
              <DropdownMenu modal={false}>
                <DropdownMenuTrigger asChild>
                  <Button variant="outline" size="sm" className="h-9 gap-1 rounded-full text-xs">
                ...
                <DropdownMenuContent className="w-56" align="end">
                  ...
                  {availableCategories.length === 0 && (
                     <DropdownMenuLabel className="text-xs font-normal text-muted-foreground">No categories found</DropdownMenuLabel>
                  )}
                </DropdownMenuContent>
              </DropdownMenu>
              ...
              <DropdownMenu modal={false}>
                <DropdownMenuTrigger asChild>
                   <Button variant="outline" size="sm" className="h-9 w-[150px] gap-1 rounded-full text-xs">
                    <SortAscending className="h-3.5 w-3.5" />
                    {sortLabels[sortCriteria]} {/* Display current sort label */}
                  </Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent align="end">
                  <DropdownMenuRadioGroup value={sortCriteria} onValueChange={(value) => setSortCriteria(value as SortOption)}>
