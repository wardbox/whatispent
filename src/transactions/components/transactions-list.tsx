...
export function TransactionsList({
  ...
  const transactionGroups: TransactionGroup[] | undefined = useMemo(() => {
    ...
    const searchedTransactions = transactions.filter((tx: TransactionWithDetails) => {
      const prettyCategory = getPrettyCategoryName(tx.category?.[0] ?? 'Uncategorized').toLowerCase();
      return (
        (tx.merchantName?.toLowerCase() || tx.name.toLowerCase()).includes(queryLower) ||
    ...
    const filteredTransactions = selectedCategories.size === 0
      ...
      : searchedTransactions.filter((tx: TransactionWithDetails) => {
          const prettyCategory = getPrettyCategoryName(tx.category?.[0] ?? 'Uncategorized');
    ...
    const sortedGroups: TransactionGroup[] = Object.entries(groups)
      ...
      const sortedTransactions = [...txs].sort((a: TransactionWithDetails, b: TransactionWithDetails) => {
        switch (sortCriteria) {
          case 'date-asc':
    ...
    .sort((a, b) => {
      ...
      return dayjs(b.transactions[0].date).valueOf() - dayjs(a.transactions[0].date).valueOf();
    });
  ...
  if (isLoadingTransactions) {
    return <div className="text-center text-muted-foreground pt-4">Loading transactions...</div>;
  }

  if (transactionsError) {
    return <div className="text-destructive text-center pt-4">Error loading transactions: {transactionsError.message}</div>;
  }

  if (!transactions || transactions.length === 0) {
    return <div className="text-center text-muted-foreground pt-4">No transactions found. Connect a bank account to get started!</div>;
  }

  if (!transactionGroups || transactionGroups.length === 0) {
    return <div className="text-center text-muted-foreground pt-4">No transactions match your criteria.</div>;
  }
    ...
    <div className="space-y-4">
      {transactionGroups.map((group) => (
        <div key={group.id} className="space-y-2">
          <div className="flex cursor-pointer items-center justify-between" onClick={() => toggleGroup(group.id)}>
            <div>
              <h3 className="text-sm font-medium">{group.title}</h3>
              <p className="text-xs text-muted-foreground">{group.date}</p>
            </div>
            <div className="text-sm font-light">
              {expandedGroup.has(group.id) ? "Hide" : "Show"} ({group.transactions.length})
            </div>
          </div>
          ...
          {expandedGroup.has(group.id) && (
            ...
            >
              {group.transactions.map((transaction, index) => {
                ...
                const displayTime = group.id === 'today' || group.id === 'yesterday' ? transactionTime : transactionDay;
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className="flex h-10 w-10 items-center justify-center rounded-full" style={{ backgroundColor: `var(${categoryColorVar})` }}>
                        <TransactionIcon
                          className="h-5 w-5 text-background"
                        />
                      </div>
                      <div>
                        <p className="text-sm font-light">{transaction.merchantName ?? transaction.name}</p>
                        <div className="flex items-center gap-2">
                          <p className="text-xs text-muted-foreground">{prettyCategory}</p>
                    ...
                    <div className="text-right">
                      <p className={`text-sm font-light ${transaction.amount > 0 ? "" : "text-green-500"}`}>
                        {transaction.amount > 0 ? "-" : "+"}$ {/* Plaid expenses are positive */}
