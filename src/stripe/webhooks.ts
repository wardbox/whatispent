...
export const stripeWebhookMiddlewareConfigFn: MiddlewareConfigFn = middlewareConfig => {
...
function mapStripeStatusToUserStatus(stripeStatus: Stripe.Subscription.Status): string {
  switch (stripeStatus) {
    case 'active':
    case 'trialing':
      return 'active';
...
export async function handleStripeWebhook(req: Request, res: Response) {
  ...
  console.debug('[handleStripeWebhook] Request headers:', JSON.stringify(req.headers, null, 2));
  // @ts-ignore - rawBody is added by middleware
  console.debug('[handleStripeWebhook] Does req.rawBody exist?:', req.hasOwnProperty('rawBody'));
  // @ts-ignore - rawBody is added by middleware
  console.debug('[handleStripeWebhook] req.rawBody type:', typeof req.rawBody);
  // @ts-ignore - rawBody is added by middleware
  console.debug('[handleStripeWebhook] req.rawBody value:', req.rawBody ? req.rawBody.toString('utf-8').substring(0, 100) + '...' : 'N/A'); // Log first 100 chars
  // --- END DEBUGGING ---
  ...
  try {
    ...
    console.debug('[handleStripeWebhook] Attempting constructEvent with req.body'); 
    console.debug('[handleStripeWebhook] req.body type:', typeof req.body);
    console.debug('[handleStripeWebhook] req.body value:', req.body ? JSON.stringify(req.body).substring(0, 100) + '...' : 'N/A'); // Log first 100 chars
  ...
  try {
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        if (session.mode === 'subscription' && session.customer && session.subscription) {
          ...
          console.log(`Updated user status for customer ${customerId} to ${userStatus} after checkout.`);
        } else if (session.mode === 'setup') {
          console.log('Setup intent session completed, no subscription status change.');
        } else {
             console.log(`Checkout session completed (mode: ${session.mode}), but not a subscription. No status update.`);
        }
        break;
      }
      ...
      case 'customer.subscription.updated': {
        ...
        console.log(`Updated user status for customer ${customerId} to ${userStatus} due to subscription update.`);
        break;
      }
      ...
      case 'customer.subscription.deleted': {
        ...
        console.log(`Updated user status for customer ${customerId} to ${userStatus} due to subscription deletion.`);
        break;
      }
    ...
    if (error.code === 'P2025') { // Prisma code for "Record to update not found."
        ...
        const customerIdGuess = (error.meta?.cause as string)?.split(' ')?.pop() || 'unknown';
        console.warn(`Webhook handler: User with Stripe Customer ID ${customerIdGuess} not found.`);
