...
export const getUsers = (async (_args, context) => {
  ...
  if (!context.user?.isAdmin) {
    throw new HttpError(
      403,
      'Unauthorized. You must be an admin to view users.',
    )
  }
  ...
  if (
    ...
    !context.entities?.Institution ||
    !context.entities?.Account
  ) {
    throw new HttpError(
      500,
      'Internal server error: Entities not available in context.',
    )
  }
  ...
  const users = await context.entities.User.findMany({
    select: {
      ...
      createdAt: true,
      institutions: {
        // We still need institutions to count accounts
        select: {
          _count: {
  ...
  return users.map((user: UserWithInstitutionCount) => {
    ...
    const totalAccounts = user.institutions.reduce(
      (sum: number, inst: InstitutionWithAccountCount) =>
        sum + (inst._count?.accounts || 0),
